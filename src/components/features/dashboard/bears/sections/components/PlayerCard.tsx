...
export default function PlayerCard({ player, isInjured = false }: PlayerCardProps) {
  return (
    ...
    >
      ...
      {isInjured && (
        <div className="absolute top-2 right-2 px-2 py-1 bg-red-500/20 border border-red-500/50 rounded text-xs font-semibold text-red-300 uppercase tracking-wide">
          Out
        </div>
      )}
      ...
      {/* Player Name */}
      <h3 className="font-semibold text-white text-sm leading-tight mb-1">{player.name}</h3>
      ...
      <div className="absolute bottom-2 right-2 text-slate-500/40 group-hover:text-slate-400/60 transition-colors">
